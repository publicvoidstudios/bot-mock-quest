...
const finalTimeout: number = 2 * 60000; //1 minute final timeout before any task expiration
...
function checkTaskTimeout(task: Task): boolean {
    const now: number = new Date().getTime();
    const createdAt: number = new Date(task.created_at).getTime();
    const diff: number = now - createdAt;
    return diff >= task.timeout;
}


const taskTotallyExpired = (task: Task): boolean => {
    const now: number = new Date().getTime();
    const createdAt: number = new Date(task.created_at).getTime();
    const expiresIn: number = (now - createdAt) - task.timeout;
    return expiresIn >= finalTimeout;
}


const sendMessage = async (user_id: number, message: Message): Promise<void> => {
...
const askLater = async (task: Task) => {
...
const clearAskLater = async (task: Task) => {
    if(task.ask_later !== null) {
        await db('tasks')
            .where('id', task.id)
            .select('ask_later')
...
const tasksCheckupInfiniteLoop = async (timeout: number) => {
...
const assignTaskToUser = async (task_id: number, user_id: number) => {
...
const declineTask = async (task_id: number, user_id: number) => {
    let task: Task | null = null;
    await db("tasks")
        .where('id', task_id)
        .select()
...
const postponeTask = async (task_id: number, user_id: number) => {
    let task: Task | null = null;
    await db("tasks")
        .where('id', task_id)
        .select()
